...
knitr::opts_chunk$set(
  ...
  warning = FALSE, 
  message = FALSE, 
  fig.path = here("docs/img/rifampin_midazolam_ddi-"), 
  cache = TRUE, 
  autodep = TRUE, 
  cache.path = here("docs/cache/rifampin")
)
...
sims %>% 
  group_by(ID) %>% 
  pmxTools::get_auc(time = "time", dv = "Cmidazolam") %>%
...
summ <- 
  ...
  group_by(rif,mid) %>%
  nest() %>%
  summarise(data = lapply(data, get_auc), .groups = "drop") %>%
  unnest(cols = data) %>% 
  mutate(pAUC = 100*(AUC/first(AUC)))
